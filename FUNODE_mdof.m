function x_dot = FUNODE_mdof(t,x,MiC,MiK,Mi,stiffness,epsilon,ndof,w,tt)

    x_dot = zeros(3*ndof,1);
    
    H = [zeros(ndof,ndof) eye(ndof) zeros(ndof,ndof);
        -MiK -MiC zeros(ndof,ndof);
        zeros(ndof,ndof) zeros(ndof,ndof) eye(ndof)];
  
    f = zeros(ndof, 1);
    g0 = zeros(ndof, 1);
    for i=1:ndof
        f(i,1) = interp1(tt,w(i,:),t,'linear');
        g0(i,1) = epsilon(i)*stiffness(i)*x(i).^3;

    end

    Mf = Mi*f;
    F = [zeros(ndof,1);Mf;zeros(ndof,1)];
    
    gm = -Mi*g0;
    G = [zeros(ndof,1);gm;zeros(ndof,1)];

    x_dot = H*x + F + G;






